...
int main() {
  struct autobus *serenity = ab__creer(3, 1);

  struct ps_standard *kaylee = ps__creer("Kaylee", 4, false);
  struct ps_standard *jayne = ps__creer("Jayne", 4, false);
  struct ps_standard *inara = ps__creer("Inara", 5, true);
  struct ps_standard *morgane = ps__creer("Morgane", 3, false);
  struct ps_standard *raph = ps__creer("Raph", 5, true);
  ...
  ps__monter_dans(morgane, serenity);

  ab__affiche_etat(serenity);
  ps__affiche_etat(kaylee);
  ps__affiche_etat(jayne);
  ps__affiche_etat(morgane); 

  //3
  ab__aller_arret_suivant(serenity);
  ps__monter_dans(inara, serenity);
  ps__monter_dans(raph, serenity);

  ab__affiche_etat(serenity);
  ps__affiche_etat(kaylee);
  ps__affiche_etat(jayne);
  ps__affiche_etat(inara);
  ps__affiche_etat(raph);
  ps__affiche_etat(morgane); 
